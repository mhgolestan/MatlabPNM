function [NumberOfClusters, NodeL, LinkL,cluster_A_nums,cluster_B_nums] = Clustering_water(network)
...
for i = 1:network.numberOfNodes
    NodeS(i,1) = network.Nodes{i}.occupancy; % nodes with oil(B),nodes with water(A)
    
    if network.Nodes{i}.occupancy == 'A'
        water = 1;
    end
    if network.Nodes{i}.shapeFactor <= 1 / 16 % There is water in the corners
        NodeS(i,1) = 'A';
    end
...
for i =1:network.numberOfLinks
    if ~network.Links{i}.isInlet && ~network.Links{i}.isOutlet
        LinkS(i,1) = network.Links{i}.occupancy; % throats with oil(B), throats with water(A)
        
        if network.Links{i}.occupancy == 'A'
            water = 1;
        end
        if network.Links{i}.shapeFactor <= 1 / 16 % There is water in the corners
            LinkS(i,1) = 'A';
        end
    end
end
...
%%
    ...
    for i =1:network.numberOfLinks
        if network.Links{i}.isInlet
            if  network.Links{i}.occupancy == 'A' || any(network.Links{i}.waterCornerExist)
