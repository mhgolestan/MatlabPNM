...
if strcmp(element.geometry , 'Circle')== 1
    ...
    if element.advancingContactAngle <= maxAdvAngle % Spontaneous imbibition
        newPc = 1.1 * element.IFT_NperMeter * 2 * cos(element.advancingContactAngle) / element.radius;
        MAX_NEWT_ITR = 50000;
        for itr = 1:MAX_NEWT_ITR
            ...
            for i = 1:nc
                ...
                b_i = element.IFT_NperMeter / oldPc * cos(hingTeta +  halfAngles(i))/sin(halfAngles(i));
                if b_i< 0
                    b_i = 0;
                end
                partus = b_i * sin(halfAngles(i)) * oldPc / element.IFT_NperMeter;
                if partus > 1
                    partus = 1;
                elseif partus < -1
                    partus = -1;
            ...
            root_s = bb ^ 2 - 4 * a  * c;
            if root_s > 0
                newPc2 = element.IFT_NperMeter * 2 * a / ( -bb + sqrt(root_s));
            else
                newPc2 = element.IFT_NperMeter * 2 * a / (-bb);
            end
            newPc = newPc2;
