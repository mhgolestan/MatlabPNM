function [NumberOfClusters, NodeL, LinkL,cluster_A_nums,cluster_B_nums] = clustering_nonWettingPhase(network)
...
for i = 1:network.numberOfNodes
    NodeS(i,1) = network.Nodes{i}.occupancy; % nodes with nonWetting(1),nodes with wettingPhase(0)
    if any(network.Nodes{i}.nonWettingLayerExist)
        NodeS(i,1)='B';
    end
...
for i =1:network.numberOfLinks
    if ~network.Links{i}.isInlet && ~network.Links{i}.isOutlet
        LinkS(i,1) = network.Links{i}.occupancy; % throats with nonWetting(1), throats with wettingPhase(0)
        if any(network.Links{i}.nonWettingLayerExist)
            LinkS(i,1)='B';
        end
    end
end
OFlag = 'B'; %nonWetting clusters has numbers 1:NumberOfClusters %wettingPhase clusters are 0
...
for i =1:network.numberOfLinks
    if network.Links{i}.isInlet
        if any(network.Links{i}.nonWettingLayerExist) || network.Links{i}.occupancy == 'B'
            if network.Nodes{network.Links{i}.pore2Index}.occupancy == 'B' || any(network.Nodes{network.Links{i}.pore2Index}.nonWettingLayerExist)
    ...
    elseif network.Links{i}.isOutlet
        if (network.Links{i}.occupancy == 'B' || any(network.Links{i}.nonWettingLayerExist))
            if network.Nodes{network.Links{i}.pore1Index}.occupancy == 'B' || any(network.Nodes{network.Links{i}.pore1Index}.nonWettingLayerExist)
